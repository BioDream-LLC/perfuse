...
func TestWorkerRetriesThenSucceeds(t *testing.T) {
	...
	//
	// count() counts successful sends only - a failure increments a separate counter and appends nothing - so waiting for
	// one was the right condition for "the fourth attempt succeeded". The race was after that: the worker sends, and then
	// records the outcome in the store. Waiting on the send and cancelling immediately could stop the worker in between, so
	// the item was still Sending with three attempts when it was read. It failed about one run in six, and only in a full
	// run where the machine was busy enough for that gap to matter.
	waitFor(t, func() bool {
		items, _, err := s.List(ctx, Query{})

		return err == nil && len(items) == 1 && items[0].State == Delivered && items[0].Attempts == 4
	})
